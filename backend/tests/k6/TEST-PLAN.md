...
:: ========== T1: 阶梯拐点测试（~25min）==========
cd backend
tests\k6\test-scripts\run-test.bat --mode performance --reset --env PROFILE=ramp --monitor
```

如需逐请求数据和可视化图表（HTML + ASCII），加 `--full` 参数：
...
> `--full` 模式会生成 JSON Lines 文件（200-500MB），可通过时间戳提取每个阶段的指标。
...
`--full` 模式下，测试完成后会自动调用 `plot-stages.js` 生成可视化：

| 输出 | 说明 |
|------|------|
| `*-stages.json` | 各阶梯详细指标（由 `parse-report.js` 生成） |
| `*-charts.html` | 交互式 HTML 图表（Chart.js，含响应时间叠加图、RPS 柱状图、接口 P95 对比） |
| Markdown 内嵌 | ASCII 图表自动追加到 `*-report.md` |
...
7. **可视化图表** — `--full` 模式会自动生成 `*-charts.html` 和追加 ASCII 图表到 Markdown
8. **Redis Key 分布** — 测试结束自动采样，无需额外操作，结果在 Markdown 报告中查看
...
| overload | 200 | ~2m | 500/2000 | 50→200→400→0 | 已由 ramp 替代 |
...
| Stages JSON | `parse-report.js`（`--full` 模式） | ~50KB | 各阶梯分段指标 |
| Markdown 报告 | `parse-report.js` | ~15KB | 可读的测试报告（含 ASCII 图表） |
| HTML 图表 | `plot-stages.js`（`--full` 模式） | ~200KB | Chart.js 交互式图表 |
