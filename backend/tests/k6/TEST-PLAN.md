...
| 模式 | 参数 | 说明 |
|------|------|------|
| **单进程**（默认） | `--reset` | 启动 1 个 Node.js 进程处理所有请求 |
| **多进程集群** | `--reset --cluster` | 启动 N 个 worker 进程（N = CPU 核数），自动负载均衡 |

> 集群模式利用多核 CPU 并行处理请求，可显著降低高并发下的 P95 延迟。单进程用于对比基准。
...
# 帕累托分布（多进程集群）
run-test.bat --mode performance --reset --cluster --env PROFILE=ramp --monitor

# 均匀分布（与 pareto 对比缓存命中率差异，单进程）
...
mini_verify（快速验证，~3min，覆盖全部 18 接口，验证脚本正确性）
        │
        ▼
T1（阶梯拐点测试 - 单进程，~25min 含 reset，pareto 分布）
        │
        ▼
  分析结果，确定：
  - 拐点 VUs（如 250）
...
7. **建议对比单进程 vs 集群** — T1 先不加 `--cluster` 跑单进程基准，再加 `--cluster` 跑集群模式，对比 P95 改善
7. **报告文件位置**：
...
:: ========== T1: 阶梯拐点测试（~25min，单进程基准）==========
cd backend
tests\k6\test-scripts\run-test.bat --mode performance --reset --env PROFILE=ramp --monitor --full

:: ========== T1: 阶梯拐点测试（~25min，多进程集群）==========
cd backend
tests\k6\test-scripts\run-test.bat --mode performance --reset --cluster --env PROFILE=ramp --monitor --full
...
|------|:--------:|:--------------------------:|
| 完整执行（mini_verify+T1单进程+T1集群+T2+T3） | 5 组 | ~87 分钟 |
| 核心测试（T1单进程+T1集群+T2） | 3 组 | ~82 分钟 |
| 核心测试（T1+T2） | 2 组 | ~57 分钟 |
| 快速验证（仅 mini_verify） | 1 组 | ~3 分钟 |
| 快速验证（仅 T1 单进程） | 1 组 | ~25 分钟 |