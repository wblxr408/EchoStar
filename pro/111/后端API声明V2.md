...
| email | 是 | 邮箱 |
| password | 是 | 新密码 |
| verificationCode | 是 | 邮箱验证码（通过 1.1 接口获取） |
...
| 字段 | 必填 | 说明 |
|------|-------|------|
| content | 是 | 故事正文，最多 1000 字 |
| images | 否 | 图片 URL 数组，1-9 张 |
| location | 是 | 经纬度对象 `{ lng, lat }` |
| emotionTag | 是 | 情绪标签：'治愈'/'难过'/'开心'/'打卡' |
| isTimeCapsule | 否 | 是否时光胶囊，默认 false |
| unlockAt | 否 | 解锁时间，格式: ISO 8601 |
| visibility | 否 | 可见性：'public'/'shadowban' |
...
| 字段 | 必填 | 说明 |
|------|-------|------|
| visibility | 是 | 可见性：'public' 或 'shadowban' |
...
| 参数 | 必填 | 说明 |
|------|-------|------|
| northEast | 是 | 东北角坐标 JSON |
| southWest | 是 | 西南角坐标 JSON |
...
| 字段 | 必填 | 说明 |
|------|-------|------|
| action | 是 | 操作类型：'approve'（批准）或 'reject'（拒绝） |
